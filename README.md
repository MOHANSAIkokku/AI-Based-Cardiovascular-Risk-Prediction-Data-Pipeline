# Heart-Disease-Prediction
Heart Disease Prediction uses machine learning to classify patients as high or low risk for heart disease based on clinical features like age, cholesterol, blood pressure, and chest pain type. This beginner-friendly project demonstrates complete ML workflow: data exploration, preprocessing, model training, evaluation, and prediction on new patients


🎓 Learning Outcomes
Complete ML workflow from raw CSV to predictions

Handle real-world medical data (missing values, categoricals)

Model selection and comparison

Medical visualization and interpretation

Production-ready code structure

🩺 Medical Impact
Early Detection: Identifies at-risk patients

Key Predictors: Cholesterol, chest pain type most important

Deployable: Easy to convert to web app for clinics

Ethical: High accuracy minimizes false alarms

🔮 Future Enhancements
 Hyperparameter tuning (GridSearchCV)

 Cross-validation

 Feature importance analysis

 Streamlit web app deployment

 Model saving/loading (joblib)

 ROC curves and precision-recall

📚 Technologies Used
Category	Tools
Language	Python 3.8+
ML Framework	scikit-learn 1.3
Data Processing	pandas, numpy
Visualization	matplotlib, seaborn
